...
local HakaiFolder = "Hakai"
local ConfigurationFolder = HakaiFolder.."/Configurations"
local ConfigurationExtension = ".hk"
...
local HakaiLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Hakai
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across Hakai
...
local Hakai = game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn.protect_gui then 
	syn.protect_gui(Hakai)
	Hakai.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Hakai.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Hakai.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Hakai.Name and Interface ~= Hakai then
			Interface.Enabled = false
			Interface.Name = "Hakai-Old"
		end
	end
else
...
local Main = Hakai.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Hakai.DisplayOrder = 100
...
local Notifications = Hakai.Notifications

local SelectedTheme = HakaiLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = HakaiLibrary.Theme[ThemeName]
	for _, obj in ipairs(Hakai:GetDescendants()) do
	...
	Hakai.Main.BackgroundColor3 = SelectedTheme.Background
	Hakai.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Hakai.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Hakai.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Hakai.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Hakai.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Hakai.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if HakaiLibrary.Flags[FlagName] then
			spawn(function() 
				if HakaiLibrary.Flags[FlagName].Type == "ColorPicker" then
					HakaiLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if HakaiLibrary.Flags[FlagName].CurrentValue or HakaiLibrary.Flags[FlagName].CurrentKeybind or HakaiLibrary.Flags[FlagName].CurrentOption or HakaiLibrary.Flags[FlagName].Color ~= FlagValue then HakaiLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			HakaiLibrary:Notify({Title = "Flag Error", Content = "Hakai was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(HakaiLibrary.Flags) do
...
function HakaiLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= HakaiLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Hakai | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	HakaiLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function HakaiLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Hakai Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Hakai Interface Suite" then
		LoadingFrame.Version.Text = "Hakai UI"
	end
	...
	pcall(function()
		...
		if not isfolder(HakaiFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(HakaiFolder.."/Discord Invites") then
			makefolder(HakaiFolder.."/Discord Invites")
		end
		if not isfile(HakaiFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(HakaiFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Hakai RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(HakaiFolder.."/Key System") then
			makefolder(HakaiFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Hakai | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(HakaiFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(HakaiFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(HakaiFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						HakaiLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Hakai | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					HakaiLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Hakai | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Hakai | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					HakaiLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Hakai | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					HakaiLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Hakai | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					HakaiLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Hakai | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					HakaiLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function HakaiLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				HakaiLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, HakaiLibrary.LoadConfiguration, HakaiLibrary)
